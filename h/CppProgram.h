...
// Phplus names that are hardcoded should start with '-' so that collisions can't happen with input code

/* naming conventions for hardcoded Phplus name prefixes
these go in frount of any names that are hardcoded for the transpiler, since they are not valid identifier
cars they can not collide with names from pp source and they will be removed before they get to C++ source
...
public:
	...
	void addPp(const string& pp, const string& cppNameHint="<- the value of that pp string please"); // will throw an error if ppName already exists
	void reserveCpp(const string& cpp, bool ignoreCollisions=false);
	bool hasPp(const string& pp);
	string getCpp(const string& pp); // will throw an error if the Phplus name doesn't exist
	CppNameContainer* getParent() {return parent;}
	
private:
	
	bool hasPpMe(const string& pp);
	CppNameContainer();
	...
	std::map<string, string> ppToCppMap;
...
public:
	...
	void name(const string& in); // converts a Phplus name to a posibly different C++ name
	...
	void pushBlock();
	void popBlock();
	string ppToCpp(const string& in);
...
public:
	...
	string ppToCpp(const string& in){return activeFunc->ppToCpp(in);}
