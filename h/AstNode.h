...
public:
	...
	void setInput(Namespace nsIn, bool dynamicIn, Type left, Type right)
	{
		if (inputHasBeenSet)
		{
			throw PhplusError("tried to set input on an AST node '"+getString()+"' more then once", INTERNAL_ERROR, getToken());
		}
	...
	{
		if (!inputHasBeenSet)
			throw PhplusError("tried to get inLeftType before input was set", INTERNAL_ERROR, getToken());
		return inLeftType;
	}
	...
	{
		if (!inputHasBeenSet)
			throw PhplusError("tried to get inRightType before input was set", INTERNAL_ERROR, getToken());
		return inRightType;
	}
	*/
	...
	Type getReturnType()
	{
		if (!returnType)
		{
			if (!inputHasBeenSet)
			{
				throw PhplusError("tried to get return type from AST node when input had not been set", INTERNAL_ERROR, getToken());
			}
		...
		if (!returnType)
		{
			throw PhplusError("AST node "+getString()+"failed to supply a return type", INTERNAL_ERROR);
		}
	...
	Action getAction()
	{
		if (!action)
		{
			if (!ns)
			{
				throw PhplusError("tried to get action from AST node when input had not been set", INTERNAL_ERROR, getToken());
			}
	...
	void dealWithConstants()
	{
		if (!ns)
		{
			throw PhplusError("tried to deal with constants before input was set", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	void resolveAction()
	{
		...
		{
			throw PhplusError("AstVoid given non void input", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	static AstNode make(AstNode leftTypeIn, AstNode rightTypeIn, AstNode returnTypeIn, AstNode bodyIn)
	{
		...
		{
			throw PhplusError("AstFuncBody made with function input nodes that are not types", INTERNAL_ERROR);
		}
...
public:
	...
	void resolveAction()
	{
		throw PhplusError("AstType::resolveAction called, which it shouldn't have been", INTERNAL_ERROR, getToken());
	}
};
...
public:
	...
	void resolveAction()
	{
		throw PhplusError("AstActionWrapper::resolveAction called, which it shouldn't have been", INTERNAL_ERROR);
	}
...
public:
	...
	void resolveAction() {throw PhplusError("AstWhatevToActionFactory::resolveAction called, wich should never happen", INTERNAL_ERROR);}
