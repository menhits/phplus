...
void CppNameContainer::addPp(const string& pp, const string& cppNameHint)
{
	string validCppHint;
	
	if (cppNameHint=="<- the value of that pp string please")
		validCppHint=getValidCppId(pp);
	else if (!cppNameHint.empty())
		validCppHint=getValidCppId(cppNameHint);
	
	if (ppToCppMap.find(pp)!=ppToCppMap.end())
	{
		throw PhplusError("Tried to add '"+pp+"' as a pp name to a CppNameContainer but that pp name already exists", INTERNAL_ERROR);
	}
	
	// now we need to find a unique C++ name, the Phplus name will almost always be unique, but there may be cases where Phplus treats scope differently or uses a keyword or something
...
void CppNameContainer::reserveCpp(const string& cpp, bool ignoreCollisions)
{
	...
	{
		throw PhplusError("called CppNameContainer::reserveCpp with id '"+cpp+"', which already exists", INTERNAL_ERROR);
	}
	
	cppSet.insert(cpp);
}

bool CppNameContainer::hasPpMe(const string& pp)
{
	return ppToCppMap.find(pp)!=ppToCppMap.end();
}

bool CppNameContainer::hasPp(const string& pp)
{
	return hasPpMe(pp) || (parent && parent->hasPp(pp));
}
...
string CppNameContainer::getCpp(const string& pp)
{
	auto result=ppToCppMap.find(pp);
	
	if (result==ppToCppMap.end())
	{
		if (parent)
			return parent->getCpp(pp);
		else
			throw PhplusError("could not find C++ equivalent of '"+pp+"' in CppNameContainer::getCppForPp", INTERNAL_ERROR);
	}
	else
...
void CppFuncBase::endln()
{
	if (exprLevel>0)
	{
		throw PhplusError("non zero expression level when ending line in C++ program, code so far:\n"+indentString(source), INTERNAL_ERROR);
	}
...
void CppFuncBase::popExpr()
{
	if (exprLevel<=0)
	{
		throw PhplusError("CppProgram::popExpression called with zero expressionLevel", INTERNAL_ERROR);
	}
...
void CppFuncBase::pushBlock()
{
	if (exprLevel>0)
	{
		throw PhplusError("CppProgram::pushBlock called when expressionLevel was not zero", INTERNAL_ERROR);
	}
...
void CppFuncBase::popBlock()
{
	
	{
		if (blockLevel<=0)
		{
			throw PhplusError("CppProgram::popBlock called with zero indentationLevel", INTERNAL_ERROR);
		}
...
string CppFuncBase::ppToCpp(const string& in)
{
...
{
	globalTopCode+="// this C++ code is transpiled from Phplus\n";
	globalTopCode+="// Phplus v"+to_string(VERSION_X)+"."+to_string(VERSION_Y)+"."+to_string(VERSION_Z)+" was used\n";
...
string CppProgram::getTypeCode(Type in)
{
	...
	case TypeBase::TUPLE:
	{
		...
		if (!globalNames->hasPp(compact))
		{
			globalNames->addPp(compact, in->nameHint);
			string code;
			code+="struct ";
	...
	default:
		throw PhplusError("CppProgram::getTypeCode called with invalid type "+(TypeBase::getString(in->getType())), INTERNAL_ERROR);
	}
}
...
void CppProgram::declareVar(const string& nameIn, Type typeIn, string initialValue)
{
	...
	*/
	...
	if (activeFunc->namespaceStack.back()->hasPp(nameIn))
	{
		return;
	}
	
	activeFunc->namespaceStack.back()->addPp(nameIn);
	activeFunc->namespaceStack[0]->reserveCpp(ppToCpp(nameIn), true);
	
	activeFunc->varDeclareSource+=getTypeCode(typeIn)+" "+ppToCpp(nameIn);
...
{
	if (globalNames->hasPp(nameIn))
		return;
	
	string code;
	code+=getTypeCode(typeIn);
	code+=" ";
	globalNames->addPp(nameIn);
...
void CppProgram::addFunc(const string& name, vector<std::pair<string, string>> args, string returnType, string contents)
{
	if (hasFunc(name))
	{
		throw PhplusError("called CppProgram::addFunc with function name '"+name+"', which already exists", INTERNAL_ERROR);
	}
	
	globalNames->addPp(name, name);
...
void CppProgram::pushFunc(const string& name, const string& cppNameHint, Type leftIn, Type rightIn, Type returnType)
{
	if (hasFunc(name))
	{
		throw PhplusError("called CppProgram::pushFunc with function name '"+name+"', which already exists", INTERNAL_ERROR);
	}
	
	globalNames->addPp(name, cppNameHint);
	...
	else
	{
		...
		for (int i=0; i<int(args.size()); i++)
		{
			...
			funcNames->addPp(args[i].second, args[i].second);
...
void CppProgram::popFunc()
{
	...
	{
		throw PhplusError("called CppProgram::popFunc when function wasn't ready", INTERNAL_ERROR);
	}
	...
	if (funcStack.empty())
	{
		throw PhplusError("called CppProgram::popFunc too many times", INTERNAL_ERROR);
	}
...
{\n\
	argc = argcIn;\n\
	argv = argvIn;\n\
	if (argc >= 2 && strcmp(argv[1], \"--running-from-Phplus\") == 0)\n\
	{\n\
		argc -= 2;\n\
		if (argc == 0)\n\
			argv = 0;\n\
