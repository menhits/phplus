...
public:
	...
	{
		throw PhplusError("tried to get the literal value of 'void;", INTERNAL_ERROR);
	}
...
public:
	...
	string getCompactString()
	{
		switch (primType)
		{
			...
			default:
				throw PhplusError("tried to make " + getString() + " compact", INTERNAL_ERROR);
		}
	}
	...
	string getCppLiteral(void * data, CppProgram * prog)
	{
		...
		switch (primType)
		{
			...
			default:
				throw PhplusError("tried to convert " + getString() + " to C++ code", INTERNAL_ERROR);
		}
	...
	size_t getSize()
	{
		switch (primType)
		{
			...
			default:
				throw PhplusError("tried to get size of " + getString(), INTERNAL_ERROR);
		}
	}
...
public:
	...
	Type actuallyIs(Type target)
	{
		if (target->getType()!=TUPLE)
		{
			...
			{
				throw PhplusError("actuallyIs called on tuple with target that is not tuple", INTERNAL_ERROR);
			}
		}
...
public:
	...
	string getCppLiteral(void * data, CppProgram * prog)
	{
		throw PhplusError("getCppLiteral called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	
	size_t getSize()
	{
		throw PhplusError("getSize called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	...
	Type getSubType()
	{
		throw PhplusError("getSubType called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
...
{
	throw PhplusError("getAllSubTypes called on type that was not a tuple", INTERNAL_ERROR);
}
...
Type TypeBase::actuallyIs(Type target)
{
	if (!matches(target))
	{
		throw PhplusError("actuallyIs called with type that doesn't match", INTERNAL_ERROR);
	}
	
	if (isWhatev())
	{
		throw PhplusError("actuallyIs not implemented properly for Whatev type", INTERNAL_ERROR);
	}
