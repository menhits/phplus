...
void ErrorHandler::log(string msg, ErrorPriority priority, Token token)
{
	...
	if (token)
	{
		cout << " di '" << token->getFile()->getFilename() << "' pada baris ke " << token->getLine() << ":" << endl;
...
PhplusError::PhplusError(string msgIn, ErrorPriority priorityIn, Token tokenIn)
{
	msg=msgIn;
	priority=priorityIn;
	token=tokenIn;
