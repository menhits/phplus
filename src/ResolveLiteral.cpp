...
Action resolveIntLiteral(Token token, Type type)
{
	...
	for (auto i=in.begin(); i!=in.end(); ++i)
	{
		if (*i<'0' || *i>'9')
		{
			error.log(string() + "bad character '" + *i + "' tersedian pada nomor '" + in + "'", SOURCE_ERROR, token);
			return nullptr;
		}
...
Action resolveDubLiteral(Token token)
{
	...
	for (auto i=in.begin(); i!=in.end(); ++i)
	{
		if (*i=='.' || *i=='_')
		{
			...
			{
				error.log(string() + "multiple decimal points tersedia pada nomor '" + in + "'", SOURCE_ERROR, token);
				return voidAction;
			}
		}
		else if (*i>='0' && *i<='9')
		...
		{
			error.log(string() + "bad character '" + *i + "' tersedia pada nomor '" + in + "'", SOURCE_ERROR, token);
			return voidAction;
		}
	}
...
Action resolveLiteral(Token token)
{
	...
	{
		throw PhplusError(FUNC+" called on token that is not a literal", INTERNAL_ERROR, token);
	}
	...
	{
		throw PhplusError("tried to make literal with invalid type of " + type->getString(), INTERNAL_ERROR, token);
	}
}
