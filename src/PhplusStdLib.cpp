...
function<void(Action inLeft, Action inRight, CppProgram* prog)> stringToLambda(string cppKode)
{
	if (cppKode.empty())
	{
		return nullptr;
	...
	return [=](Action inLeft, Action inRight, CppProgram* prog)
	{
		...
		do
		{
			i=searchInString(cppKode, "$", start);
			
			if (i<0)
			{
				prog->code(cppKode.substr(start, cppKode.size()-start));
			}
			else
			{
				prog->code(cppKode.substr(start, i-start));
				
				if (substringMatches(cppKode, i, "$."))
				{
					prog->pushExpr();
				...
				else if (substringMatches(cppKode, i, "$:"))
				{
					prog->pushExpr();
				...
				{
					throw PhplusError("invalid '$' escape in C++ code: "+cppKode, INTERNAL_ERROR);
				}
			}
...
inline T getValFromTuple(void* data, Type type, string name)
{
	...
	if (!a.type)
		throw PhplusError("tried to get invalid property '"+name+"' from type "+type->getString(), INTERNAL_ERROR);
...
inline T getValFromTuple(void* data, Type type, int index)
{
	if (index<0 || index>=type->getAllSubTypes()->size())
		throw PhplusError("tried to get invalid property #"+to_string(index)+" from tuple "+type->getString(), INTERNAL_ERROR);
...
inline void setValInTuple(void* data, Type type, string name, T val)
{
	...
	if (!a.type)
		throw PhplusError("tried to set invalid property '"+name+"' from type "+type->getString(), INTERNAL_ERROR);
...
void addToProgPpStr(CppProgram * prog)
{
	if (!prog->hasFunc("$ppStr"))
	{
		string strType=prog->getTypeCode(String);
		
		prog->addFunc("$ppStr", {{"const char *", "in"}}, strType,
			"int size = 0;\n"
			"while (in[size]) size++;\n"
...
void addToProgDoubleToStr(CppProgram * prog)
{
	if (!prog->hasFunc("$doubleToStr"))
	{
		...
		string intToStr=prog->ppToCpp("$intToStr");
		string concat=prog->ppToCpp("$concatStr");
		string ppStr=prog->ppToCpp("$ppStr");
		...
		prog->addFunc("$doubleToStr", {{"double", "in"}}, prog->getTypeCode(String),
			...
			"if (b % 10 == 9)\n\tb += 1;\n"
			"while (b>0 && !(b % 10))\n\tb /= 10;\n"
			"return "+concat+"("+concat+"("+intToStr+"((int)in), "+ppStr+"(\".\")), "+intToStr+"(b));\n"
		);
	}
}
...
void addToProgGetInputLine(CppProgram * prog)
{
	if (!prog->hasFunc("$getInputLine"))
	{
		...
		prog->addFunc("$getInputLine", {{strType, "prompt"}}, strType,
			"fputs("+prog->ppToCpp("$cStr")+"(prompt), stdout);\n"
			"fflush(stdout);\n"
...
void addToProgRunCmd(CppProgram * prog)
{
	if (!prog->hasFunc("$runCmd"))
	{
		...
		prog->addFunc("$runCmd", {{strType, "cmd"}}, strType,
			strType+" result = "+prog->ppToCpp("$ppStr")+"(\"\");\n"
			...
			"	if ((c=getc(pipe)) != EOF)\n"
			"	{\n"
			"		result="+prog->ppToCpp("$concatStr")+"(result, "+prog->ppToCpp("$asciiToStr")+"(c));\n"
			"	}\n"
			"}\n"
			"pclose(pipe);\n"
			"return result;\n"
...
void populateConstants()
{
	...
	bool trueVal=true;
	addConst(&trueVal, Bool, "benar");
	
	bool falseVal=false;
	addConst(&falseVal, Bool, "salah");
...
void populateStdFuncs()
{
	...
	func("cetak", Void, Void, Void,
		cout << endl;
	,
		"fputs(\"\\n\", stdout)"
	);
	
	func("cetak", Void, Bool, Void,
		cout << (right?"benar":"salah") << endl;
	,
		"fputs($:?\"benar\\n\":\"salah\\n\", stdout)"
	);
	
	func("cetak", Void, Byte, Void,
		cout << right << endl;
	,
		"printf(\"%c\", $:)"
	);
	
	func("cetak", Void, Int, Void,
		cout << right << endl;
	,
		"printf(\"%d\\n\", $:)"
	);
	
	func("cetak", Void, Dub, Void,
	...
	addAction("cetak", Void, String, Void,
		LAMBDA_HEADER
		{
		...
		AstWhatevToActionFactory::make(
			[](Type leftType, Type rightType) -> Action
			{
				...
				return lambdaAction(
					...
					{
						void* ppStr=cppStr2PncnStr(val);
						constGetAction(ppStr, String, val, globalNamespace)->addToProg(prog);
						free(ppStr);
					},
					"typeName"
				);
...
void populateStringFuncs()
{
	...
	addAction("String", Bool, Void, String,
		LAMBDA_HEADER
		{
			if (*((bool*)leftIn))
			{
				return cppStr2PncnStr("benar");
			}
			else
			{
				return cppStr2PncnStr("salah");
			}
		},
		ADD_CPP_HEADER
		{
	...
	addAction("ascii", Int, Void, String,
		LAMBDA_HEADER
		{
			...
			if (val<0 || val>=256)
			{
				throw PhplusError("tried to make ascii string out of value "+to_string(val), RUNTIME_ERROR);
			}
			string out;
			out+=(char)val;
	...
	addAction("at", String, Int, Int,
		LAMBDA_HEADER
		{
			...
			{
				throw PhplusError("tried to access location "+to_string(index)+" in string "+to_string(str.size())+" long", RUNTIME_ERROR);
			}
			*out=str[index];
			return out;
		},
	...
	addAction("sub", String, makeTuple(vector<NamedType>{NamedType{"a", Int}, NamedType{"b", Int}}, true), String,
		LAMBDA_HEADER
		{
			...
			{
				throw PhplusError("invalid arguments sent to String.sub", RUNTIME_ERROR);
			}
		...
		AstWhatevToActionFactory::make(
			[](Type leftType, Type rightType) -> Action
			{
				...
				return lambdaAction(
					...
					[=](Action inLeft, Action inRight, CppProgram* prog)
					{
						throw PhplusError("not yet implemented", INTERNAL_ERROR);
					},
				...
				return lambdaAction(
					...
					[=](void* leftIn, void* rightIn) -> void*
					{
						...
						if (*(int*)rightIn<0 || *(int*)rightIn>=size)
							throw PhplusError("index out of bounds, tried to get element at position "+to_string(*(int*)rightIn)+" in array "+to_string(size)+" long", RUNTIME_ERROR);
				...
				return lambdaAction(
					...
					[=](void* leftIn, void* rightIn) -> void*
					{
						...
						if (index<0 || index>=size)
							throw PhplusError("index out of bounds, tried to set element at position "+to_string(index)+" in array "+to_string(size)+" long", RUNTIME_ERROR);
...
void populateIntArrayAndFuncs()
{
	...
	addAction("get", IntArray, Int, Int, LAMBDA_HEADER
		{
			...
			if (pos<0 || pos>=size)
				throw PhplusError("tried to acces position "+to_string(pos)+" of array "+to_string(size)+" long", RUNTIME_ERROR);
	...
	addAction("set", IntArray, PNCN_Tuple(Int, Int), Void,
		LAMBDA_HEADER
		{
			...
			int size=getValFromTuple<int>(leftIn, IntArray, "_size");
			if (pos<0 || pos>=size)
				throw PhplusError("tried to set value at position "+to_string(pos)+" of array "+to_string(size)+" long", RUNTIME_ERROR);
...
{
	addAction("cppKode", Void, String, Void, LAMBDA_HEADER
		{
			throw PhplusError("you can't run interpreter with code that uses 'cppKode'", SOURCE_ERROR);
		},
		ADD_CPP_HEADER
		{
			prog->pushBlock();
	...
	addAction("cppHead", Void, String, Void, LAMBDA_HEADER
		{
			throw PhplusError("you can't run interpreter with code that uses 'cppHead'", SOURCE_ERROR);
		},
		ADD_CPP_HEADER
		{
