...
public:
	...
	void* execute(void* inLeft, void* inRight)
	{
		if (!(*stackPtrPtr))
		{
			throw PhplusError("something fucked up big time. VarGetAction::execute called while stack pointer is still null", RUNTIME_ERROR);
		}
...
public:
	...
	void* execute(void* left, void* right)
	{
		if (!(*stackPtrPtr))
		{
			throw PhplusError("something fucked up big time. VarSetAction::execute called while stack pointer is still null", RUNTIME_ERROR);
		}
	...
	void addToProg(Action inLeft, Action inRight, CppProgram* prog)
	{
		...
		if ((nameHint=="me" || nameHint=="in") && getReturnType()->getType()==TypeBase::TUPLE)
		{
			...
			{
				throw PhplusError("can not set 'in' or 'me' inside expression in C++ because of some really dumb reason", INTERNAL_ERROR);
			}
	...
	void addToProg(Action inLeft, Action inRight, CppProgram* prog)
	{
		if (getReturnType()==String)
		{
			...
			prog->pushExpr();
				...
				{
					throw PhplusError("ConstGetAction::addToProg failed to access string properties", INTERNAL_ERROR);
				}
